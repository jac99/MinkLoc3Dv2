...
from models.losses.smooth_ap import SmoothAPLoss
from models.losses.truncated_recall_surrogate import TruncatedRecallSurrogateLoss
...
def make_losses(params: TrainingParams):
    if params.loss == 'batchhardtripletmarginloss':
        ...
        loss_fn = BatchHardTripletLossWithMasks(params.margin, params.embedding_mixup)
    elif params.loss == 'batchhardcontrastiveloss':
        loss_fn = BatchHardContrastiveLossWithMasks(params.pos_margin, params.neg_margin, params.embedding_mixup)
    elif params.loss == 'smoothap':
        # Loss from "Smooth-AP: Smoothing the Path Towards Large-Scale Image Retrieval" paper
        loss_fn = SmoothAPLoss(tau1=params.tau1, similarity=params.similarity)
    elif params.loss == 'truncatedsmoothap':
        loss_fn = TruncatedSmoothAP(tau1=params.tau1, similarity=params.similarity,
                                    positives_per_query=params.positives_per_query, margin=params.margin)
    elif params.loss == 'truncatedrecallsurrogate':
        # Modified version of the loss from "Recall@k Surrogate Loss with Large Batches and Similarity Mixup" paper
        loss_fn = TruncatedRecallSurrogateLoss(tau1=params.tau1, tau2=params.tau2, k=params.k,
...
class BatchHardTripletLossWithMasks:
    def __init__(self, margin:float, embedding_mixup: bool = False):
        self.margin = margin
    ...
    def __call__(self, embeddings, positives_mask, negatives_mask):
        ...
        if self.embedding_mixup:
            # Check how many negatives are taken from mixed embeddings (from the second half of the embeddings tensor)
            stats['num_mixed_negatives'] = torch.sum(hard_triplets[2] >= len(embeddings) // 2).item()

        return loss, stats


class BatchHardContrastiveLossWithMasks:
    def __init__(self, pos_margin: float, neg_margin: float, embedding_mixup: bool = False):
