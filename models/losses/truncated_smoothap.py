...
# but only the fixed number of closest positives is considered
...
class TruncatedSmoothAP:
    ...
    def __call__(self, embeddings, positives_mask, negatives_mask):
        ...
        # Compute the denominator in Eq. 5 - add sum of Indicator function for negatives
        if self.margin is not None:
            s_sigmoid = sigmoid(s_diff + self.margin, temp=self.tau1)
