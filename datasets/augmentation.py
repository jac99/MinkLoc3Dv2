    ...
    def __init__(self, aug_mode):
        ...
        if self.aug_mode == 1:
            ...
            t = [JitterPoints(sigma=0.1, clip=0.2), RemoveRandomPoints(r=(0.0, 0.1)),
                ...
                # Augmentations with 180 degree rotation around z-axis (with p=0.5)
                # So traversing the same place in both directions should give the same embedding
                t = [JitterPoints(sigma=0.1, clip=0.2), RemoveRandomPoints(r=(0.0, 0.1)),
                     RotationByAngle(thetas=[0., 180.]), RandomTranslation(max_delta=0.3), RemoveRandomBlock(p=0.4)]
        else:
...
class TrainSetTransform:
    def __init__(self, aug_mode):
        ...
        if self.aug_mode == 1:
            ...
            t = [RandomRotation(max_theta=20, axis=np.array([0, 0, 1])),
                 RandomFlip([0.25, 0.25, 0.])]
        elif self.aug_mode == 4:
            t = [RandomRotation(max_theta=5, axis=np.array([0, 0, 1])),
                 RandomFlip([0.33, 0.33, 0.]),
                 RandomScale(0.8, 1.2)]
            ...
            t = [RotationByAngle(thetas=[0., 180.]),
                 RandomRotation(max_theta=15, axis=np.array([0, 0, 1])),
                 RandomFlip([0.33, 0.33, 0.]),
                 RandomScale(0.7, 1.3)]
        else:
            raise NotImplementedError('Unknown aug_mode: {}'.format(self.aug_mode))
...
class VirtualNegativeTransform:
    # Transformation to generate 'virtual negatives'
    def __init__(self):
        t = [RotationByAngle(thetas=[0., 180.]),
             RandomFlip([0.33, 0.33, 0.]),
             RandomScale(0.8, 1.2)]
...
class RotationByAngle:
    def __init__(self, thetas: List[float]):
        # Rotation around z-axis with an angle chosen randomly from the list
        self.axis = np.array([0, 0, 1])
        self.thetas = thetas      # List of possible rotation angles

    def _M(self, axis, theta):
        return expm(np.cross(np.eye(3), axis / norm(axis) * theta)).astype(np.float32)

    def __call__(self, coords):
        theta = random.choice(self.thetas)
        R = self._M(self.axis, np.pi * theta / 180.)
        coords = coords @ R
        return coords
